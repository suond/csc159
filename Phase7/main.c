...
int main() {
   ...
   pid = DeQ(&free_q);
   StartProcISR(pid,(unsigned int)FileService);
   pid = DeQ(&free_q);
   StartProcISR(pid,(unsigned int)ShellProc);
   pid = DeQ(&free_q);
   StartProcISR(pid,(unsigned int)StdinProc);
   pid = DeQ(&free_q);
   StartProcISR(pid,(unsigned int)StdoutProc);
